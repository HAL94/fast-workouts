...
async def get_muscle_groups(
    pagination: MuscleGroupListReadPagination = Query(...),
    muscle_group_service: MuscleGroupService = Depends(
...
async def get_muscle_group_by_id(
    ...
    muscle_group = await muscle_group_service.get_muscle_groups_by_id(muscle_group_id=muscle_group_id)

    return AppResponse(data=muscle_group)


@router.get("/{muscle_group_id}/exercises")
async def get_exercises_by_muscle_group(muscle_group_id: int, muscle_group_service: MuscleGroupService = Depends(get_muscle_group_service)):
    exercises = await muscle_group_service.get_exercises_by_muscle_group(muscle_group_id)
